...
The playground runs the real ``nano_llm`` package inside Pyodide. Because the package
...
def main() -> int:
    ...
    package_dir = script_root / "sdk" / "nano_llm"
    if not package_dir.exists():
        print("bundle_sdk_for_playground.py: sdk package not found")
        return 2

    sources = {
        f"nano_llm/{module.name}": module.read_text(encoding="utf-8")
